...
import datasets
#from datasets import load_metric #Edit - replacing with Evaluate, as load_metric was depricated in datasets 3.0.0
import evaluate
...
device: torch.device, eval_dataloader: DataLoader, inference_times: list) -> tuple:
    ...
    metric = evaluate.load("accuracy") #load_metric("accuracy") #Edit to use evaluate library instead of datasets library
...
def print_and_save_model(total_predictions: torch.Tensor, total_references: torch.Tensor, checkpoint_path: str, metric) -> None:
    ...
    f_1_metric = evaluate.load("f1", trust_remote_code=True) #load_metric("f1", trust_remote_code=True) #Edit for switch to Evaluate library instead of datasets
