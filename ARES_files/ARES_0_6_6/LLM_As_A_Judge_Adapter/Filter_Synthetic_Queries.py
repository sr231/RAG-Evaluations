...
def generate_index(dataframe: pd.DataFrame) -> Dataset:
    ...
    dataframe['embeddings'] = dataframe["document"].progress_apply(lambda x: get_embedding(x, model='models/text-embedding-004')) # edited to use Gemini instead of OpenAI text-embedding-ada-002
    
    # Filter out rows where the embedding length is not 768 # was 1536 for OpenAI
...
def filter_synthetic_queries(queries_dataset: pd.DataFrame, document_index) -> pd.DataFrame:
    ...
    for i in tqdm(range(len(queries_dataset))):
        ...
        embedding = get_embedding(question, model='models/text-embedding-004')
        ...
        # Ensure question_embedding is a 2D array with shape (1, 768) # was 1536 for OpenAI embeddings
...
number_of_negatives_added_ratio: float, lower_bound_for_negatives: int) -> pd.DataFrame:
    ...
    for i in tqdm(range(len(queries_dataset_copy))):
        ...
        question_embedding = np.array(get_embedding(question, model='models/text-embedding-004')).astype(np.float32)
        
        # Ensure question_embedding is a 2D array with shape (1, 768) # was 1536 for OpenAI embeddings
        if question_embedding.shape != (768,):
            raise ValueError(f"Expected embedding of shape (768,), but got {question_embedding.shape}")
